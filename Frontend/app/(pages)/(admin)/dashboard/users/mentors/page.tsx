...
export default function MentorPage() {
  ...
  const [viewingMentor, setViewingMentor] = useState<any>(null);
  ...
  const endIndex = Math.min(startIndex + itemsPerPage, filteredMentors.length);
  ...
  const openEditModal = (mentor: any) => {
    ...
    setPassword("");
    setImageFile(null);
    setImagePreview(getAvatarUrl(mentor?.file ?? undefined));
    setNameError(false);
    setPasswordError(false);
    setPhoneError(false);
    setImageError(false);
  ...
  const handleArchiveMentor = async () => {
    if (!deletingId) return;
    try {
      await archiveMentor(deletingId);
      setMentors((prevMentor) => prevMentor.filter((a) => a.id !== deletingId));
      if (currentMentors.length === 1 && currentPage > 1) {
  ...
  const handleRestoreMentor = async () => {
    ...
    try {
      await restoreMentor(restoringId);
      setMentors((prev) => prev.filter((a) => a.id !== restoringId));
  ...
  const handleDeleteMentorPermanently = async () => {
    if (!deletingId) return;
    try {
      await deleteMentor(deletingId);
      setMentors((prev) => prev.filter((a) => a.id !== deletingId));
      if (currentMentors.length === 1 && currentPage > 1) {
        setCurrentPage((prev) => prev - 1);
      }
                      ...
                      .map((mentor) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-4 border border-gray-200">
                            ...
                            >
                              <img
                                src={getAvatarUrl(mentor?.file ?? undefined)}
                                alt={mentor?.fullName}
                          ...
                          <td className="px-5 py-4 border border-gray-200 relative">
                            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center justify-center gap-2 whitespace-nowrap">
                              {viewMode === "active" ? (
                                ...
                                <>
                                  <button
                                    onClick={() => confirmRestore(mentor.id)}
                                  ...
                                  <button
                                    onClick={() =>
                                      confirmPermanentDelete(mentor.id)
                                    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#00000099] backdrop-blur-xs">
          <div className="bg-white rounded-xl shadow-xl p-6 w-100 animate-in fade-in zoom-in duration-200">
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              <button
                onClick={handleDeleteMentorPermanently}
      ...
      {isRestoreModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#00000099] backdrop-blur-xs">
          <div className="bg-white rounded-xl shadow-xl p-6 w-100 animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-gray-600 text-sm mb-6">
              Haqiqatan ham tiklamoqchimisiz? Mentor qaytadan faol ro’yxatga
              qaytariladi.
            </p>
            <div className="flex items-center justify-end gap-3">
              <button
          ...
          >
            ...
            <div className="p-6">
              ...
              <div className="flex items-center gap-4 mb-8">
                <img
                  src={getAvatarUrl(viewingMentor?.file ?? undefined)}
                  alt={viewingMentor.fullName}
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-[14px]">Mentor</p>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-5 mb-8">
                ...
                <div>
                  ...
                  <p className="text-[15px] font-bold text-gray-900">
                    {formatDate(viewingMentor.created_at)}
                  </p>
                </div>
              </div>
